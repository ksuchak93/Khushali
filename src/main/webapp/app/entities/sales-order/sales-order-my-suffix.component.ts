...
@Component({
    selector: 'jhi-sales-order-my-suffix',
    templateUrl: './sales-order-my-suffix.component.html'
})
export class SalesOrderMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/sales-order-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/sales-order-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: SalesOrderMySuffix) {
        return item.id;
    }
